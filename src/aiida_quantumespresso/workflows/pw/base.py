...
class PwBaseWorkChain(ProtocolMixin, BaseRestartWorkChain):
    ...
    def handle_electronic_convergence_not_reached(self, calculation):
        ...
        scf_accuracy_slope, scf_accuracy_intercept = Polynomial.fit(numpy.arange(0, len(scf_accuracy)), numpy.log(scf_accuracy), 1).convert().coef
        ...
        low_symmetry_structure = (
            len(calculation.outputs.output_parameters.get_dict()['symmetries']) < self.defaults.    
        )
        ...
        if self.ctx.inputs.parameters['ELECTRONS']['electron_maxstep'] < self.defaults.max_electronic_steps:
            electron_steps_new = min(self.defaults.max_electronic_steps,[ numpy.log(scf_accuracy) - scf_accuracy_intercept] / scf_accuracy_slope)
            self.ctx.inputs.parameters['ELECTRONS']['electron_maxstep'] = electron_steps_new
            self.report(
                f'Changed the number of electron maxsteps: increasing number of electron maxsteps {electron_steps_new}'
            )
